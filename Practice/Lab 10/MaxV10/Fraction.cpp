...
Fraction* Fraction::Parse(std::string line) {
	std::string delim = "/";
	...
	cnt = line.length() - startPos;
	token = line.substr(startPos, cnt);
	numbers.push_back(std::stoi(token));
...
Fraction* Fraction::TryParse(std::string line) {
	try {
		if (!isValidString(line)) {
			throw "Invalid string";
		} else {
			return Fraction::Parse(line);
		}
	} catch (const char* message) {
		std::cout << message << "\n";
